...
namespace na
{
	...
	{
		NA_ALIGN(float v[4], 16) = { x, y, z, w };
		mElems = _mm_load_ps(v);
	}

	Vector::Vector(const Vector2f &vector)
	{
		NA_ALIGN(float v[4], 16) = { vector.x, vector.y, 0.0f, 0.0f };
		mElems = _mm_load_ps(v);
	}

	Vector::Vector(const Vector3f &vector)
	{
		NA_ALIGN(float v[4], 16) = { vector.x, vector.y, vector.z, 0.0f };
		mElems = _mm_load_ps(v);
	}

	Vector::Vector(const Vector4f &vector)
	{
		NA_ALIGN(float v[4], 16) = { vector.x, vector.y, vector.z, vector.w };
		mElems = _mm_load_ps(v);
	}
