...
namespace na
{
	...
	{
		return std::chrono::duration_cast<std::chrono::duration<float>>(mDuration).count();
	}
	...
	void Timer::Start()
	{
		NA_ASSERT_RETURN(mInterval != INVALID_INTERVAL, "You must initialize the timer before using it.");
